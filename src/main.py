...
def process_csv(csv_file_path: str, chunk_size: int, rule_list):
  ...
  for batch in batched_df:
    if len(batch.columns) != 24:
      logging.warning("Incorect number of columns. Batch is skipped")
      continue
    ...
    batch["date"] = pd.to_datetime(batch["date"], unit="s", errors='coerce')
    #prefilter
    batch = batch[(batch["date"].notna()) & (batch["severity"] == "Error")]
...
def main():
    ...
    rule2 = SlidingWindowRule(limit = 10, time_delta_seconds=3600, col_names_dict=col_names_dict, groups=["bundle_id"])
